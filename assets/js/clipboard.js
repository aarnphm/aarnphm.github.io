...
const addCopyButtons = () => {
  ...
  for (let i = 0; i < els.length; i++) {
    ...
    const codeBlocks = els[i].getElementsByTagName("code");
    ...
    button.addEventListener("click", () => {
      navigator.clipboard.writeText(
        lastCodeBlock.innerText.replace(/\n\n/g, "\n"),
      ).then(
        () => {
          button.blur();
        ...
        (e) => (button.innerHTML = "Error"),
      );
    });
    // find chroma inside els[i]
    const chroma = els[i].getElementsByClassName("chroma")[0];
