...
test('source watcher includes top-level Quartz source inputs', () => {
  ...
  assert.equal(sourceWatchPatterns.includes('.claude/skills/**/*'), true)
  assert.equal(sourceWatchRoots.includes('quartz'), true)
  assert.equal(sourceWatchRoots.includes('.claude/skills'), true)
})
...
test('source watcher accepts newly added Quartz source files', () => {
  ...
  assert.equal(isSourceWatchPath('.claude/skills/core/SKILL.md'), true)
