...
class MLA(nn.Module):
  ...
  def __init__(
    ...
    self.q_rope_proj = nn.Linear(
      hidden_dim, n_heads * rope_head_dim, bias=False
    )
    ...
    self.k_decompress = nn.Linear(
      kv_latent_dim, n_heads * head_dim, bias=False
    )
  ...
  def forward(
    ...
    q_rope = self.q_rope_proj(x).view(
      batch, seq, self.n_heads, self.rope_head_dim
    )
    q_rope = apply_rotary_pos_emb(
      q_rope,
      cos[..., : self.rope_head_dim // 2],
      sin[..., : self.rope_head_dim // 2],
    )

    # kv: compress then decompress
    c_kv = self.kv_compress(
      x
    )  # [batch, seq, latent_dim] - THIS is what we cache
    k_main = self.k_decompress(c_kv).view(
...
class NSA(nn.Module):
  ...
  def forward(
    ...
    k_compressed = self.compress_pool(k.transpose(1, 3)).transpose(
      1, 3
    )  # [b, seq//r, h, d]
    ...
    out_compressed = torch.einsum(
      'bhst,bthd->bshd', attn_compressed, v_compressed
    )
    ...
    block_scores_reshaped = block_scores.view(
      batch, self.n_heads, -1, self.block_size
    ).mean(dim=-1)

    # top-k block selection
    _, top_indices = block_scores_reshaped.topk(
      min(self.n_select_blocks, n_blocks), dim=-1
    )
    ...
    select_mask = torch.zeros(
      batch, self.n_heads, seq, seq, device=x.device, dtype=torch.bool
    )
    ...
    attn_selected = F.softmax(
      full_attn.masked_fill(~select_mask, -1e9), dim=-1
    )
    ...
    slide_mask = (
      torch
      .ones(seq, seq, device=x.device, dtype=torch.bool)
      .triu(-self.slide_window)
      .tril(0)
    )
...
class DSA(nn.Module):
  ...
  def __init__(
    ...
    self.k_index = nn.Linear(
      kv_latent_dim, index_dim, bias=False
    )  # shared across heads
  ...
  def lightning_index(
    ...
    k_idx = apply_rotary_pos_emb(k_idx.unsqueeze(2), cos_idx, sin_idx).squeeze(
      2
    )
  ...
  def forward(
    ...
    k_abs = self.k_decompress(c_kv).view(
      batch, seq, self.n_heads, self.head_dim
    )
    v = self.v_decompress(c_kv).view(batch, seq, self.n_heads, self.head_dim)

    k_r = self.k_rope(x).view(batch, seq, 1, self.rope_head_dim)
    k_r = apply_rotary_pos_emb(
      k_r,
      cos[..., : self.rope_head_dim // 2],
      sin[..., : self.rope_head_dim // 2],
    )
    ...
    full_score = (score_abs + score_r) / math.sqrt(
...
class MLA_MQA(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    attn = F.softmax(
      torch.einsum('bqhd,bkhd->bhqk', q, k.expand(-1, -1, self.n_heads, -1))
      / math.sqrt(self.head_dim),
      dim=-1,
    )
    out = torch.einsum(
      'bhqk,bkhd->bqhd', attn, v.expand(-1, -1, self.n_heads, -1)
    )
    return self.o_proj(out.reshape(batch, seq, -1))
...
class MLA_GQA(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    attn = F.softmax(
      torch.einsum('bqhd,bkhd->bhqk', q, k) / math.sqrt(self.head_dim), dim=-1
    )
    out = torch.einsum('bhqk,bkhd->bqhd', attn, v)
    return self.o_proj(out.reshape(batch, seq, -1))
