...
class NSA(nn.Module):
  ...
  def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    ...
    attn_compressed = F.softmax(torch.einsum('bshd,bthd->bhst', q, k_compressed) * scale, dim=-1)
    ...
    out = (
      g_comp.unsqueeze(-1) * out_compressed + g_sel.unsqueeze(-1) * out_selected + g_slide.unsqueeze(-1) * out_slide
    )
...
class MLA_MQA(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    attn = F.softmax(
      torch.einsum('bqhd,bkhd->bhqk', q, k.expand(-1, -1, self.n_heads, -1)) / math.sqrt(self.head_dim), dim=-1
    )
