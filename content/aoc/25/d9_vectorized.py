...
def parse(path: str) -> np.ndarray: return np.array([list(map(int, line.split(','))) for line in pathlib.Path(path).read_text().strip().split('\n')])

def p1(pts: np.ndarray) -> int: return int(((np.abs(pts[:, 0, None] - pts[None, :, 0]) + 1) * np.abs(pts[:, 1, None] - pts[None, :, 1]) + 1).max())
