...
>
> Tensor Cores compute entire matrix tiles per instruction, not scalar operations. They are not programmable; they are only accessible via specific matrix instructions (WMMA, WGMMA).
...
> For comprehensive coverage including descriptor creation, PTX instructions, mbarrier synchronization, multicast/reduction operations, and performance comparisons, see the [[#tensor-memory-accelerator-tma|dedicated TMA section]].
...
[^warp]: Warps of 32 threads execute in lockstep (SIMT). When they access consecutive memory addresses (coalesced), the hardware issues a single wide transaction, where one 128-byte load serves the entire warp. Strided access forces 32 separate transactions, killing bandwidth. Always structure access patterns so adjacent threads read adjacent addresses. This is why column-major access in row-major layouts is disastrous.
...
1. Bijection (if cosize = size and strides form a linearly independent basis or shape-aligned strides to prevent collisions): $f_{\mathcal{L}}$ is bijective on its domain
...
_Proof._ (1) If cosize = size and strides form a linearly independent basis/shape-aligned strides, then $f_{\mathcal{L}}$ maps size distinct coordinates to size distinct addresses, hence bijective. (2) Follows from linearity of dot product. (3) Follows from definition. $\square$
...
> [!theorem] Online Softmax Correctness Theorem
>
