...
tags:
  - seed
  - ml
description: quick notes on attention. See also https://aarnphm.xyz/thoughts/Attention
date: "2025-08-21"
modified: 2025-09-12 12:54:26 GMT-04:00
...
- Notation.
  - Sequence $X\in\mathbb{R}^{n\times d}$.
  - Projections $W_Q,W_K,W_V$.
  - Heads $h$, head dim $d_h$.
...
- Softmax/LSE.
  ...
  - $$
    ...
    $$
  - Stable softmax:
    - Use $z\leftarrow z-\max(z)$ (log‑sum‑exp trick).
  - temperature $T$:
    - $\operatorname{softmax}(z/T)$ controls entropy smoothly;
    - $T\uparrow\Rightarrow$ higher entropy. (Theory & practice refs.) ([Cross Validated][5], [arXiv][6])
    - softmax = $\nabla$LSE.
      - Jacobian $J=\lambda(\operatorname{Diag}(p)-pp^\top)$;
      - Lipschitz constant scales with inverse temperature parameter $\lambda$.
        - Useful for error bounds later.
- RoPE adds relative position by rotating $(q,k)$ in 2‑D frequency planes before the dot product. [@su2023roformerenhancedtransformerrotary]
- Why the $\frac{1}{\sqrt{d_k}}$ scale?
  - variance‑normalizes dot products so logits don’t blow up with $d_k$
  - (see Prop. A1 below; and [@vaswani2023attentionneed; §3.2.1])
- [[thoughts/Kullback-Leibler divergence]]
  - $$
    D_{\mathrm{KL}}(P\|Q)=\sum_i P_i\log\frac{P_i}{Q_i}
    $$
  - Jointly convex in $(P,Q)$; bounds total variation via Pinsker.
    1. align attention distributions,
    2. measure head agreement,
    3. distill approximations (e.g., MHA -> GQA). ([home.ttic.edu][3], [people.lids.mit.edu][4])
...
> [!abstract] Proposition A.1
>
> _permutation equivariance_
>
...
Row‑softmax respects row permutations, i.e., $\sigma(PZP^\top)=P\sigma(Z)P^\top$.

Then $P\sigma(QK^\top)P^\top \cdot PV = P(\sigma(QK^\top)V) \; \boxed{}$.

> [!abstract] Proposition A.2
>
> _why the $\tfrac{1}{\sqrt{d_k}}$ scale? [^read]_
>
...
[^read]:
    Assume $q,k\in\mathbb{R}^{d_k}$ have i.i.d. entries with $\mathbb{E}[q_i]=\mathbb{E}[k_i]=0$, $\operatorname{Var}(q_i)=\operatorname{Var}(k_i)=1$. Then $\operatorname{Var}(q^\top k)=d_k$.
    Scaling by $1/\sqrt{d_k}$ keeps logit variance $\approx 1$, stabilizing softmax and gradients as width grows. @vaswani2023attentionneed
...
two‑pass Triton kernel:

- pass 1: computes running row‑max/sum for softmax
- pass 2 recomputes tiles to form the output $O$ (avoids storing $S$).

> Single head, no mask, one batch for studying.

![[lectures/2/attention_triton.py]]

## where to push performance?
