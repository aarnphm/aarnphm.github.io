...
def fetch_matrix(
  state: dict[str, Any], key_template: str, layer: int
) -> torch.Tensor:
...
def slice_value_head(
  mat: torch.Tensor, head: int, num_heads: int
) -> torch.Tensor:
  rows, cols = mat.shape
  if rows % num_heads == 0:
    span = rows // num_heads
  ...
  raise ValueError(
    'value projection shape is incompatible with the requested head count'
  )


def slice_output_head(
  mat: torch.Tensor, head: int, num_heads: int
) -> torch.Tensor:
  rows, cols = mat.shape
  if cols % num_heads == 0:
  ...
  if rows % num_heads == 0:
    ...
    return mat[start : start + span, :].T
  raise ValueError(
    'output projection shape is incompatible with the requested head count'
  )
...
def compute_w_ov(
  ...
  if o_head.shape[1] != v_head.shape[0]:
    if o_head.shape[0] == v_head.shape[0]:
      ...
      raise RuntimeError(
        f'cannot align output/value projections: O head {o_head.shape}, V head {v_head.shape}'
      )
...
def latent_error(
  w_ov: torch.Tensor, latent_dim: int
...
def find_key_for_component(
  state: dict[str, Any], layer: int, patterns: list[str]
) -> str | None:
  lowered_items = [
    (key, key.lower()) for key in state.keys() if matches_layer(key, layer)
  ]
...
def infer_templates(state: dict[str, Any], layer: int = 0) -> dict[str, str]:
  ...
  if qkv_key:
    ...
    q_key = find_key_for_component(
      state, layer, ['q_proj.weight', 'wq.weight', 'query.weight']
    )
    k_key = find_key_for_component(
      state, layer, ['k_proj.weight', 'wk.weight', 'key.weight']
    )
    v_key = find_key_for_component(
      state, layer, ['v_proj.weight', 'wv.weight', 'value.weight']
    )
    if not (q_key and k_key and v_key):
      raise KeyError(
        'Unable to infer Q/K/V key templates; please specify them manually.'
      )
  ...
  o_key = find_key_for_component(
    state,
    layer,
    ['o_proj.weight', 'out_proj.weight', 'wo.weight', 'proj.weight'],
  )
  if not o_key:
...
def select_head_count(length: int) -> int:
  ...
  for d in candidates:
    ...
    if score < best_score or (
      score == best_score and (best is None or d > best)
    ):
      best = d
      best_score = score
...
def main() -> None:
  ...
  parser.add_argument(
    '--weights', type=Path, help='checkpoint path (.pt/.bin/.safetensors)'
  )
  parser.add_argument(
    '--hf-repo',
    type=str,
  ...
  parser.add_argument(
    '--v-key-template',
    type=str,
    default='transformer.h.{layer}.attn.v_proj.weight',
  )
  parser.add_argument(
    '--o-key-template',
    type=str,
    default='transformer.h.{layer}.attn.o_proj.weight',
  )
  parser.add_argument(
    '--qkv-key-template',
    type=str,
    default=None,
    help='optional combined qkv key template',
  )
  parser.add_argument(
    '--auto-infer',
    action='store_true',
    help='infer key templates from the state dict',
  )
  ...
  parser.add_argument(
    ...
    type=int,
    default=32,
    help='synthetic tokens for cache reconstruction',
  )
  parser.add_argument('--seed', type=int, default=0)
  parser.add_argument(
    '--plot',
    action='store_true',
    help='save a spectrum/cumulative energy plot',
  )
  parser.add_argument(
    '--plot-path',
    type=Path,
  ...
  parser.add_argument(
    ...
    type=str,
    default=None,
    help='text prompt for attention heatmap (HF models only)',
  )
  parser.add_argument(
  ...
  parser.add_argument(
    '--heatmap',
    action='store_true',
    help='save an attention heatmap for the prompt',
  )
  parser.add_argument(
    '--heatmap-path',
    type=Path,
    default=None,
    help='output path for the attention heatmap',
  )
  ...
  num_heads, num_value_heads = resolve_head_counts(
    num_heads, num_value_heads, config, v_matrix, o_matrix
  )
  ...
  w_ov = compute_w_ov(
    v_matrix, o_matrix, args.head, num_heads, num_value_heads
  )
  ...
  cache_err = torch.linalg.norm(projected - truncated) / torch.linalg.norm(
    projected
  )

  print(
    f'layer={args.layer} head={args.head} num_heads={num_heads} num_value_heads={num_value_heads}'
  )
  print(
    f'W_OV shape: {tuple(w_ov.shape)} rank: {torch.linalg.matrix_rank(w_ov)}'
  )
  print(f'condition number: {cond:.3e} effective_rank: {r_eff:.2f}')
  print(
    f'latent_dim={args.latent_dim} rel_error={rel_err:.3e} cache_error={cache_err.item():.3e}'
  )
  ...
  if args.plot:
    ...
    plot_path = args.plot_path or Path(
      ...
      'thoughts',
      'images',
      f'{stem}_layer{args.layer}_head{args.head}.png',
    )
  ...
  if args.heatmap:
    if model_obj is None or prompt_text is None:
      raise ValueError(
        'Heatmap generation requires --hf-repo with an accessible model and prompt text'
      )
    try:
    ...
    if args.layer >= len(attentions):
      raise ValueError(
        f'layer index {args.layer} exceeds available layers ({len(attentions)})'
      )

    layer_attn = attentions[args.layer]
    if args.head >= layer_attn.shape[1]:
      raise ValueError(
        f'head index {args.head} exceeds available heads ({layer_attn.shape[1]})'
      )
    ...
    heatmap_path = args.heatmap_path or Path(
      ...
      'thoughts',
      'images',
      f'{stem}_layer{args.layer}_head{args.head}_heatmap.png',
    )
