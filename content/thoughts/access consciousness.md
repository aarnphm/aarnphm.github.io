---
date: '2026-01-28'
description: information availability, global workspace, and what interpretability can actually map
id: access-consciousness
modified: 2026-05-15 01:01:20 GMT-04:00
seealso:
  - '[[thoughts/phenomenal consciousness]]'
  - '[[thoughts/dualism]]'
  - '[[thoughts/functionalism]]'
  - '[[thoughts/qualia]]'
  - '[[thoughts/Attention]]'
  - '[[thoughts/mechanistic interpretability]]'
socials:
  sep: https://plato.stanford.edu/entries/consciousness-access/
collaborators:
  - gemini
  - codex
tags:
  - philosophy
  - consciousness
title: access consciousness
---

_access consciousness is the part a lab can catch: the report, the button press, the signal routed into later control._

- access consciousness tracks information passing through a bottleneck
- it becomes available for verbal report, reasoning, and behavioral control
- it maps to the part third-person methods and [[thoughts/mechanistic interpretability]] can study

### block's distinction (1995)

- **access consciousness (a-consciousness)**: availability for reasoning, control, report
- **[[thoughts/phenomenal consciousness]] (p-consciousness)**: experiential feel (what it is like)
- block argues researchers conflate these, explaining access while claiming to explain experience

### global workspace theory

- baars (1988) and dehaene (2011) propose consciousness as a spotlight
- illuminated items broadcast to unconscious processors
- competing neural coalitions fight for access
- winning signals ignite via prefrontal cortex, anterior temporal lobe, inferior parietal lobe, and precuneus
- explains the broadcast, leaves the feel untouched

### dissociation cases

- **blindsight**: damage to primary visual cortex (V1) leaves patients guessing correctly on visual tasks without [[thoughts/pdfs/Weiskrantz.pdf|visual experience]]
  - visual information bypasses V1 via subcortical pathways
  - local extrastriate read-out remains possible while global integration breaks
- **overflow experiments**: sperling (1960) showed subjects see 12-letter arrays but report 3-4
  - block (2011) argues experience overflows access
  - cohen and dennett counter that subjects hold generic representations until cued
- **inattentional blindness**: simons & chabris (1999) invisible gorilla experiment
  - attention acts as a necessary condition for access consciousness

### conflation problem

- chalmers notes researchers explain reportability and declare consciousness solved
- frankish rejects the hard problem entirely
- chalmers calls this eliminativism

### interpretability limits

- [[thoughts/mechanistic interpretability]] maps features, circuits, information flow, and input-output causality
- yields a third-person map of access-level structure
- says where the red-token goes, not whether red feels like anything

### moral status

- sentientist view: moral status requires [[thoughts/phenomenal consciousness]]
- access-based view: representational agency grounds moral status
- epistemic uncertainty applies to AI if phenomenal consciousness is necessary
