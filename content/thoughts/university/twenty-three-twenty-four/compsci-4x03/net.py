...
def parse():
  return (data := loadmat('points.mat'))['x'], data['labels']
...
def accuracy(a4, labels):
  return np.mean(np.argmax(a4, axis=0) == np.argmax(labels, axis=0)) * 100
...
def backward(x, labels, a2, a3, a4, W3, W4):
  ...
  gradb4, gradb3, gradb2 = (
    np.sum(d4, axis=1, keepdims=True) / m,
    np.sum(d3, axis=1, keepdims=True) / m,
    np.sum(d2, axis=1, keepdims=True) / m,
  )
  ...
  for count in range(epochs):
    ...
    if count % 1000 == 0 or count == epochs - 1:
      ...
      if accuracies[count] >= 95:
        ...
        break
    if count % decay_step == 0:
      eta *= decay_rate

  # Plots
  if DEBUG:
    plot_decision_boundary(x, labels, W2, b2, W3, b3, W4, b4)
    plot_relations(count, costs, accuracies)
  return 0
...
def plot_relations(count, costs, accuracies):
  ...
  plt.figure()
  plt.subplot(2, 1, 1)
  plt.plot(costs[: count + 1])
  plt.title('Cost over iterations')
  plt.ylabel('Cost')
  plt.subplot(2, 1, 2)
  plt.plot(accuracies[: count + 1])
...
if __name__ == '__main__':
  np.random.seed(420)
  DEBUG = os.getenv('DEBUG', str(False)).upper() == 'TRUE'
