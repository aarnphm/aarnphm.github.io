...
# Output is the estimated y for both X and X_all
def solve_kernel_ls(
  K_train: npt.NDArray[np.float64],
  Y_train: npt.NDArray[np.float64],
  K_test: npt.NDArray[np.float64],
  alpha: float,
):
  a = np.dot(
    np.linalg.pinv(K_train + alpha * np.identity(np.shape(X_train)[0])),
    Y_train,
  )
...
def gaussian_kernel(
  X1: npt.NDArray[np.float64], X2: npt.NDArray[np.float64], sigma: float
):
  ...
  K0 = (
    np.tile(norms_X2, (n, 1)) + np.tile(norms_X1, (m, 1)).T - 2 * (X1 @ X2.T)
  )
...
def run_ols(
  ...
  print(
    'Mean Squarred Error (MSE) of train data:',
    np.square(np.linalg.norm(Y_LS_train - Y_train)) / Y_train.size,
  )
  print(
    'Mean Squarred Error (MSE) of test data:',
    np.square(np.linalg.norm(Y_LS_test - Y_test)) / Y_test.size,
  )
...
if __name__ == '__main__':
  ...
  run_ols(
    X_train, Y_train, X_test, Y_test, alpha, X_train, X_test, 'Homogenous line'
  )
  ...
  run_ols(
    K, Y_train, K_test, Y_test, alpha, X_train, X_test, 'Gaussian Kernel', True
  )
