...
NiFpga_Status NiFpga_Open(const char *path, const char *signature,
                          const char *resource, uint32_t attribute,
  ...
  if (NiFpga_acquireCviResource && NiFpga_IsNotError(result))
    NiFpga_acquireCviResource((void *)*session, NiFpga_cviResourceType,
                              "NiFpga_Session %#08x", *session);
#endif
  return result;
}
...
static NiFpga_Status(NiFpga_CCall *NiFpga_readSgl)(NiFpga_Session session,
                                                   uint32_t indicator,
                                                   float *value) = NULL;

NiFpga_Status NiFpga_ReadSgl(NiFpga_Session session, uint32_t indicator,
                             float *value) {
  return NiFpga_readSgl ? NiFpga_readSgl(session, indicator, value)
                        : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readDbl)(NiFpga_Session session,
                                                   uint32_t indicator,
                                                   double *value) = NULL;

NiFpga_Status NiFpga_ReadDbl(NiFpga_Session session, uint32_t indicator,
                             double *value) {
  return NiFpga_readDbl ? NiFpga_readDbl(session, indicator, value)
                        : NiFpga_Status_ResourceNotInitialized;
}
...
static NiFpga_Status(NiFpga_CCall *NiFpga_writeSgl)(NiFpga_Session session,
                                                    uint32_t control,
                                                    float value) = NULL;

NiFpga_Status NiFpga_WriteSgl(NiFpga_Session session, uint32_t control,
                              float value) {
  return NiFpga_writeSgl ? NiFpga_writeSgl(session, control, value)
                         : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeDbl)(NiFpga_Session session,
                                                    uint32_t control,
                                                    double value) = NULL;

NiFpga_Status NiFpga_WriteDbl(NiFpga_Session session, uint32_t control,
                              double value) {
  return NiFpga_writeDbl ? NiFpga_writeDbl(session, control, value)
                         : NiFpga_Status_ResourceNotInitialized;
}
...
static NiFpga_Status(NiFpga_CCall *NiFpga_readArraySgl)(NiFpga_Session session,
                                                        uint32_t indicator,
                                                        float *array,
                                                        size_t size) = NULL;

NiFpga_Status NiFpga_ReadArraySgl(NiFpga_Session session, uint32_t indicator,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_readArrayDbl)(NiFpga_Session session,
                                                        uint32_t indicator,
                                                        double *array,
                                                        size_t size) = NULL;

NiFpga_Status NiFpga_ReadArrayDbl(NiFpga_Session session, uint32_t indicator,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_writeArrayBool)(
    NiFpga_Session session, uint32_t control, const NiFpga_Bool *array,
    size_t size) = NULL;
...
static NiFpga_Status(NiFpga_CCall *NiFpga_writeArraySgl)(NiFpga_Session session,
                                                         uint32_t control,
                                                         const float *array,
                                                         size_t size) = NULL;

NiFpga_Status NiFpga_WriteArraySgl(NiFpga_Session session, uint32_t control,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_writeArrayDbl)(NiFpga_Session session,
                                                         uint32_t control,
                                                         const double *array,
                                                         size_t size) = NULL;

NiFpga_Status NiFpga_WriteArrayDbl(NiFpga_Session session, uint32_t control,
                                   const double *array, size_t size) {
...
#if NiFpga_CviResourceTracking
  if (NiFpga_acquireCviResource && NiFpga_IsNotError(result))
    NiFpga_acquireCviResource(*context, NiFpga_cviResourceType,
                              "NiFpga_IrqContext 0x%p", *context);
#endif
  return result;
}
...
static NiFpga_Status(NiFpga_CCall *NiFpga_waitOnIrqs)(
    ...
    uint32_t timeout, uint32_t *irqsAsserted, NiFpga_Bool *timedOut) = NULL;

NiFpga_Status NiFpga_WaitOnIrqs(NiFpga_Session session,
                                NiFpga_IrqContext context, uint32_t irqs,
                                uint32_t timeout, uint32_t *irqsAsserted,
                                NiFpga_Bool *timedOut) {
  return NiFpga_waitOnIrqs ? NiFpga_waitOnIrqs(session, context, irqs, timeout,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_configureFifo2)(
    NiFpga_Session session, uint32_t fifo, size_t requestedDepth,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoBool)(
    NiFpga_Session session, uint32_t fifo, NiFpga_Bool *data,
...
NiFpga_Status NiFpga_ReadFifoBool(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_readFifoBool
             ? NiFpga_readFifoBool(session, fifo, data, numberOfElements,
                                   timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoI8)(
    NiFpga_Session session, uint32_t fifo, int8_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoI8(NiFpga_Session session, uint32_t fifo,
                                int8_t *data, size_t numberOfElements,
                                uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoI8
             ? NiFpga_readFifoI8(session, fifo, data, numberOfElements, timeout,
                                 elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoU8)(
    NiFpga_Session session, uint32_t fifo, uint8_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoU8(NiFpga_Session session, uint32_t fifo,
                                uint8_t *data, size_t numberOfElements,
                                uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoU8
             ? NiFpga_readFifoU8(session, fifo, data, numberOfElements, timeout,
                                 elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoI16)(
    NiFpga_Session session, uint32_t fifo, int16_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoI16(NiFpga_Session session, uint32_t fifo,
                                 int16_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoI16
             ? NiFpga_readFifoI16(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoU16)(
    NiFpga_Session session, uint32_t fifo, uint16_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoU16(NiFpga_Session session, uint32_t fifo,
                                 uint16_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoU16
             ? NiFpga_readFifoU16(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoI32)(
    NiFpga_Session session, uint32_t fifo, int32_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoI32(NiFpga_Session session, uint32_t fifo,
                                 int32_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoI32
             ? NiFpga_readFifoI32(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoU32)(
    NiFpga_Session session, uint32_t fifo, uint32_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoU32(NiFpga_Session session, uint32_t fifo,
                                 uint32_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoU32
             ? NiFpga_readFifoU32(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoI64)(
    NiFpga_Session session, uint32_t fifo, int64_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoI64(NiFpga_Session session, uint32_t fifo,
                                 int64_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoI64
             ? NiFpga_readFifoI64(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoU64)(
    NiFpga_Session session, uint32_t fifo, uint64_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoU64(NiFpga_Session session, uint32_t fifo,
                                 uint64_t *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoU64
             ? NiFpga_readFifoU64(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoSgl)(
    NiFpga_Session session, uint32_t fifo, float *data, size_t numberOfElements,
    uint32_t timeout, size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoSgl(NiFpga_Session session, uint32_t fifo,
                                 float *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoSgl
             ? NiFpga_readFifoSgl(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_readFifoDbl)(
    NiFpga_Session session, uint32_t fifo, double *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_ReadFifoDbl(NiFpga_Session session, uint32_t fifo,
                                 double *data, size_t numberOfElements,
                                 uint32_t timeout, size_t *elementsRemaining) {
  return NiFpga_readFifoDbl
             ? NiFpga_readFifoDbl(session, fifo, data, numberOfElements,
                                  timeout, elementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}
...
static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoBool)(
    NiFpga_Session session, uint32_t fifo, const NiFpga_Bool *data,
    size_t numberOfElements, uint32_t timeout,
...
NiFpga_Status NiFpga_WriteFifoBool(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoBool
             ? NiFpga_writeFifoBool(session, fifo, data, numberOfElements,
                                    timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoI8)(
    NiFpga_Session session, uint32_t fifo, const int8_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoI8(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoI8
             ? NiFpga_writeFifoI8(session, fifo, data, numberOfElements,
                                  timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoU8)(
    NiFpga_Session session, uint32_t fifo, const uint8_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoU8(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoU8
             ? NiFpga_writeFifoU8(session, fifo, data, numberOfElements,
                                  timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoI16)(
    NiFpga_Session session, uint32_t fifo, const int16_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoI16(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoI16
             ? NiFpga_writeFifoI16(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoU16)(
    NiFpga_Session session, uint32_t fifo, const uint16_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoU16(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoU16
             ? NiFpga_writeFifoU16(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoI32)(
    NiFpga_Session session, uint32_t fifo, const int32_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoI32(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoI32
             ? NiFpga_writeFifoI32(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoU32)(
    NiFpga_Session session, uint32_t fifo, const uint32_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoU32(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoU32
             ? NiFpga_writeFifoU32(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoI64)(
    NiFpga_Session session, uint32_t fifo, const int64_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoI64(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoI64
             ? NiFpga_writeFifoI64(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoU64)(
    NiFpga_Session session, uint32_t fifo, const uint64_t *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;
...
NiFpga_Status NiFpga_WriteFifoU64(NiFpga_Session session, uint32_t fifo,
  ...
  return NiFpga_writeFifoU64
             ? NiFpga_writeFifoU64(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoSgl)(
    NiFpga_Session session, uint32_t fifo, const float *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;

NiFpga_Status NiFpga_WriteFifoSgl(NiFpga_Session session, uint32_t fifo,
                                  const float *data, size_t numberOfElements,
                                  uint32_t timeout,
                                  size_t *emptyElementsRemaining) {
  return NiFpga_writeFifoSgl
             ? NiFpga_writeFifoSgl(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_writeFifoDbl)(
    NiFpga_Session session, uint32_t fifo, const double *data,
    size_t numberOfElements, uint32_t timeout,
    size_t *emptyElementsRemaining) = NULL;

NiFpga_Status NiFpga_WriteFifoDbl(NiFpga_Session session, uint32_t fifo,
                                  const double *data, size_t numberOfElements,
                                  uint32_t timeout,
                                  size_t *emptyElementsRemaining) {
  return NiFpga_writeFifoDbl
             ? NiFpga_writeFifoDbl(session, fifo, data, numberOfElements,
                                   timeout, emptyElementsRemaining)
             : NiFpga_Status_ResourceNotInitialized;
}

static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsBool)(
    NiFpga_Session session, uint32_t fifo, NiFpga_Bool **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsU8)(
    NiFpga_Session session, uint32_t fifo, uint8_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsU8(NiFpga_Session session, uint32_t fifo,
                                 uint8_t **elements, size_t elementsRequested,
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsI16)(
    NiFpga_Session session, uint32_t fifo, int16_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsI16(NiFpga_Session session, uint32_t fifo,
                                  int16_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsU16)(
    NiFpga_Session session, uint32_t fifo, uint16_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsU16(NiFpga_Session session, uint32_t fifo,
                                  uint16_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsI32)(
    NiFpga_Session session, uint32_t fifo, int32_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsI32(NiFpga_Session session, uint32_t fifo,
                                  int32_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsU32)(
    NiFpga_Session session, uint32_t fifo, uint32_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsU32(NiFpga_Session session, uint32_t fifo,
                                  uint32_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsI64)(
    NiFpga_Session session, uint32_t fifo, int64_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsI64(NiFpga_Session session, uint32_t fifo,
                                  int64_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsU64)(
    NiFpga_Session session, uint32_t fifo, uint64_t **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsU64(NiFpga_Session session, uint32_t fifo,
                                  uint64_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsSgl)(
    NiFpga_Session session, uint32_t fifo, float **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status NiFpga_AcquireFifoReadElementsSgl(NiFpga_Session session,
                                                uint32_t fifo, float **elements,
                                                size_t elementsRequested,
                                                uint32_t timeout,
                                                size_t *elementsAcquired,
                                                size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_acquireFifoReadElementsDbl)(
    NiFpga_Session session, uint32_t fifo, double **elements,
    size_t elementsRequested, uint32_t timeout, size_t *elementsAcquired,
    size_t *elementsRemaining) = NULL;

NiFpga_Status
NiFpga_AcquireFifoReadElementsDbl(NiFpga_Session session, uint32_t fifo,
                                  double **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsI8(NiFpga_Session session, uint32_t fifo,
                                  int8_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsU8(NiFpga_Session session, uint32_t fifo,
                                  uint8_t **elements, size_t elementsRequested,
                                  uint32_t timeout, size_t *elementsAcquired,
                                  size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsI16(NiFpga_Session session, uint32_t fifo,
                                   int16_t **elements, size_t elementsRequested,
                                   uint32_t timeout, size_t *elementsAcquired,
                                   size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsI32(NiFpga_Session session, uint32_t fifo,
                                   int32_t **elements, size_t elementsRequested,
                                   uint32_t timeout, size_t *elementsAcquired,
                                   size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsI64(NiFpga_Session session, uint32_t fifo,
                                   int64_t **elements, size_t elementsRequested,
                                   uint32_t timeout, size_t *elementsAcquired,
                                   size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsSgl(NiFpga_Session session, uint32_t fifo,
                                   float **elements, size_t elementsRequested,
                                   uint32_t timeout, size_t *elementsAcquired,
                                   size_t *elementsRemaining) {
...
NiFpga_Status
NiFpga_AcquireFifoWriteElementsDbl(NiFpga_Session session, uint32_t fifo,
                                   double **elements, size_t elementsRequested,
                                   uint32_t timeout, size_t *elementsAcquired,
                                   size_t *elementsRemaining) {
...
static NiFpga_Status(NiFpga_CCall *NiFpga_getBitfileContents)(
    NiFpga_Session session, const char **contents) = NULL;
...
static NiFpga_Status(NiFpga_CCall *NiFpga_clientFunctionCall)(
    NiFpga_Session session, uint32_t group, uint32_t functionId,
...
NiFpga_Status NiFpga_ClientFunctionCall(NiFpga_Session session, uint32_t group,
  ...
  return NiFpga_clientFunctionCall
             ? NiFpga_clientFunctionCall(session, group, functionId, inBuffer,
...
static const struct {
  ...
  NiFpga_FunctionPointer *const address;
    ...
    {"NiFpgaDll_ReadArrayBool",
     (NiFpga_FunctionPointer *)&NiFpga_readArrayBool},
    ...
    {"NiFpgaDll_WriteArrayBool",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayBool},
    ...
    {"NiFpgaDll_WriteArrayI16",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayI16},
    {"NiFpgaDll_WriteArrayU16",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayU16},
    {"NiFpgaDll_WriteArrayI32",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayI32},
    {"NiFpgaDll_WriteArrayU32",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayU32},
    {"NiFpgaDll_WriteArrayI64",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayI64},
    {"NiFpgaDll_WriteArrayU64",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayU64},
    {"NiFpgaDll_WriteArraySgl",
     (NiFpga_FunctionPointer *)&NiFpga_writeArraySgl},
    {"NiFpgaDll_WriteArrayDbl",
     (NiFpga_FunctionPointer *)&NiFpga_writeArrayDbl},
    {"NiFpgaDll_ReserveIrqContext",
     (NiFpga_FunctionPointer *)&NiFpga_reserveIrqContext},
    {"NiFpgaDll_UnreserveIrqContext",
     (NiFpga_FunctionPointer *)&NiFpga_unreserveIrqContext},
    {"NiFpgaDll_WaitOnIrqs", (NiFpga_FunctionPointer *)&NiFpga_waitOnIrqs},
    {"NiFpgaDll_AcknowledgeIrqs",
     (NiFpga_FunctionPointer *)&NiFpga_acknowledgeIrqs},
    {"NiFpgaDll_ConfigureFifo",
     (NiFpga_FunctionPointer *)&NiFpga_configureFifo},
    {"NiFpgaDll_ConfigureFifo2",
     (NiFpga_FunctionPointer *)&NiFpga_configureFifo2},
    ...
    {"NiFpgaDll_WriteFifoBool",
     (NiFpga_FunctionPointer *)&NiFpga_writeFifoBool},
    ...
    {"NiFpgaDll_AcquireFifoReadElementsBool",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsBool},
    {"NiFpgaDll_AcquireFifoReadElementsI8",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsI8},
    {"NiFpgaDll_AcquireFifoReadElementsU8",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsU8},
    {"NiFpgaDll_AcquireFifoReadElementsI16",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsI16},
    {"NiFpgaDll_AcquireFifoReadElementsU16",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsU16},
    {"NiFpgaDll_AcquireFifoReadElementsI32",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsI32},
    {"NiFpgaDll_AcquireFifoReadElementsU32",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsU32},
    {"NiFpgaDll_AcquireFifoReadElementsI64",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsI64},
    {"NiFpgaDll_AcquireFifoReadElementsU64",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsU64},
    {"NiFpgaDll_AcquireFifoReadElementsSgl",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsSgl},
    {"NiFpgaDll_AcquireFifoReadElementsDbl",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoReadElementsDbl},
    {"NiFpgaDll_AcquireFifoWriteElementsBool",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsBool},
    {"NiFpgaDll_AcquireFifoWriteElementsI8",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsI8},
    {"NiFpgaDll_AcquireFifoWriteElementsU8",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsU8},
    {"NiFpgaDll_AcquireFifoWriteElementsI16",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsI16},
    {"NiFpgaDll_AcquireFifoWriteElementsU16",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsU16},
    {"NiFpgaDll_AcquireFifoWriteElementsI32",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsI32},
    {"NiFpgaDll_AcquireFifoWriteElementsU32",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsU32},
    {"NiFpgaDll_AcquireFifoWriteElementsI64",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsI64},
    {"NiFpgaDll_AcquireFifoWriteElementsU64",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsU64},
    {"NiFpgaDll_AcquireFifoWriteElementsSgl",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsSgl},
    {"NiFpgaDll_AcquireFifoWriteElementsDbl",
     (NiFpga_FunctionPointer *)&NiFpga_acquireFifoWriteElementsDbl},
    {"NiFpgaDll_ReleaseFifoElements",
     (NiFpga_FunctionPointer *)&NiFpga_releaseFifoElements},
    {"NiFpgaDll_GetPeerToPeerFifoEndpoint",
     (NiFpga_FunctionPointer *)&NiFpga_getPeerToPeerFifoEndpoint},
    {"NiFpgaDll_GetBitfileContents",
     (NiFpga_FunctionPointer *)&NiFpga_getBitfileContents},
    {"NiFpgaDll_ClientFunctionCall",
     (NiFpga_FunctionPointer *)&NiFpga_clientFunctionCall},
    {NULL, NULL}};
...
#elif NiFpga_VxWorks
      SYM_TYPE type;
      if (symFindByName(sysSymTbl, (char *)name, (char **)address, &type) != OK)
        return NiFpga_Status_VersionMismatch;
#elif NiFpga_Linux || NiFpga_MacOsX
...
/* enable CVI Resource Tracking, if available */
#if NiFpga_CviResourceTracking
    {
      ...
      if (engine) {
        NiFpga_acquireCviResource = (NiFpga_AcquireCviResource)GetProcAddress(
            engine, "__CVI_Resource_Acquire");
        NiFpga_releaseCviResource = (NiFpga_ReleaseCviResource)GetProcAddress(
