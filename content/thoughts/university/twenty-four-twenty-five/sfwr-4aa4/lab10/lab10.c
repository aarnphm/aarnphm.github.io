...
int main(int argc, char **argv)
{
    ...
    double errorC=0, errorP=0;  //the error for current step and for previous step
    double tolerance=0.5; //error tolerance, than 0.5 Degree,

    double eInt=0;
    double eDiff;  // for PID calculation, the error integral, the error differential b
    ...
    if (argc<5) {
    		printf("Please provide a setpoint and the three PID parameters, Kp, Ki and Kd\n");
    		return (-1);
    }else {
