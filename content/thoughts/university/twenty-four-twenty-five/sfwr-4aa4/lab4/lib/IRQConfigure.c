...
void Irq_Wait(NiFpga_IrqContext irqContext, NiFpga_Irq irqNumber,
  ...
  /*
   * Break the infinite timeout up into an finite loop calling
   * NiFpga_WaitOnIrqs() with a finite timeout, and check if the thread should
   * stop every loop.
   */
  do {
    /*
     * This is a blocking function that stops the calling thread until the FPGA
     * asserts any IRQ in the number parameter, or until the function call times
     * out. Before calling this function, use NiFpga_ReserveIrqContext to
     * reserve an IRQ context. No other threads can use the same context when
     * this function is called. You can use the irqsAsserted parameter to
     * determine which IRQs were asserted for each function call.
     *
     */
...
void Irq_Acknowledge(uint32_t irqAssert) {
   ...
   */
  MyRio_ReturnIfNotSuccess(status, "Could not acknowledge Irqs!")
}
