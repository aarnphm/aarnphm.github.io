...
def input_embedding_backward(
  x: npt.NDArray[np.float32], W_E: npt.NDArray[np.float32], dOut: npt.NDArray[np.float32]
) -> tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]:
...
def softmax_backward_from_probs(
  p: npt.NDArray[np.float32], grad_out: npt.NDArray[np.float32], axis=-1
) -> npt.NDArray[np.float32]:
...
def softmax_backward(
  scores: npt.NDArray[np.float32], grad_out: npt.NDArray[np.float32], axis=-1
) -> npt.NDArray[np.float32]:
  return softmax_backward_from_probs(softmax(scores, axis=axis), grad_out)
...
def qkv_projection_backward(
  ...
  dXq, dW_Q = matmul_backward(X2D, W_Q, dOut2D)
  dXk, dW_K = matmul_backward(X2D, W_K, dOut2D)
  dXv, dW_V = matmul_backward(X2D, W_V, dOut2D)
...
def feed_forward_network(x: npt.NDArray[np.float32], W_1: npt.NDArray[np.float32], W_2: npt.NDArray[np.float32]):
  return relu(x @ W_1) @ W_2


def feed_forward_network_backward(
  x: npt.NDArray[np.float32], W_1: npt.NDArray[np.float32], W_2: npt.NDArray[np.float32], dOut: npt.NDArray[np.float32]
) -> tuple[npt.NDArray[np.float32], npt.NDArray[np.float32], npt.NDArray[np.float32]]:
  B, S, d_model = x.shape
...
def block_forward(
  ...
  *,
  ...
  q, k, v = qkv_projection(x, W_Q, W_K, W_V)
  attn_o = multi_head_attention(q, k, v, causal=causal, attn_mask=attn_mask) @ W_O
...
def block_backward(
  ...
  *,
  ...
  q, k, v = qkv_projection(x, W_Q, W_K, W_V)
  attn_pre = multi_head_attention(q, k, v, causal=causal, attn_mask=attn_mask)
  ...
  if q.ndim == 3:
    dQ4, dK4, dV4 = multi_head_attention_backward(
      q[:, None], k[:, None], v[:, None], dAttnPre[:, None], causal=causal, attn_mask=attn_mask
    )
    dQ, dK, dV = dQ4[:, 0], dK4[:, 0], dV4[:, 0]
  else:
    dQ, dK, dV = multi_head_attention_backward(q, k, v, dAttnPre, causal=causal, attn_mask=attn_mask)
