...
@app.function(**config)
def triton_add_remote(
  size: int = 98432, block_size: int = 1024, seed: int = 0
):
  ...
  def add_host(x_tensor, y_tensor):
    ...
    add_kernel[grid](
      x_tensor, y_tensor, output, n_elements_local, BLOCK_SIZE=block_size
    )
    return output
  ...
  print(
    f'[remote] Triton add: size={size}, block_size={block_size}, max_diff={max_diff}'
  )
  return {
    'size': int(size),
...
def main(size: int = 98432, block_size: int = 1024, seed: int = 0):
  result = triton_add_remote.remote(
    size=size, block_size=block_size, seed=seed
  )
