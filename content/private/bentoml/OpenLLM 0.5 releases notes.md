...
> [!NOTE] Users depending on `openllm.LLM` > `openllm.LLM` is vassly different v0.4.x internally. If your old service still depends on the older BentoML service architecture, make sure to pass in `IMPLEMENTATION=deprecated` to keep your service from breaking. We encourage you to upgrade your service to the BentoML 1.2 architecture at your convenience.
