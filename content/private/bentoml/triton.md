...
easy-to-use and framwork-agnostic APIs for creating maching learning services
and brings your models to production. It also supports various deployments
architectures (microservices, container,
[kubernetes](https://github.com/bentoml/Yatai), etc.)

## Why using Triton as a runner makes sense?

[Runners](https://docs.bentoml.org/en/latest/concepts/runner.html) are
responsible for executing the model inference logic on either CPU or GPU.
Runners represent a unit of computation and can scale independently of the API
Servers.

Instances of API Servers and Runners are deployed in separate containers during
production. From a developer experience perspective, remote invocation of
runners looks and feels like invoking another Python module.

This makes Runners ideal abstractions for integration with other runtime, such
as Triton Inference Server.

We built this integration with Triton Inference Server as we want to bring
Triton's performance to BentoML's Runner performance. One of the reasons why we
choose Triton is that the framework is written in C++ and not Python, therefore
it triumphs over its Python counterpart in terms of hardware utilization and
performance.
...
[Slack](https://l.linklyhq.com/l/ktOX) channel! optimization library to aid this
process.
