---
date: '2026-07-14'
description: épisode 8, bilan d'hiver, vérifiez vos progrès et à vous de jouer
id: episode-8
...
tags:
  ...
  - language
  - learning
title: 'épisode 8 : bilan'
---

the hiver bilan runs on the same two devices as [[fr/episode-4]]: `Voyons voir !`, under `VÉRIFIEZ VOS PROGRÈS`, self-assesses the four skill blocks of épisodes 5 à 7, and `À vous de jouer !` swaps the automne snakes-and-ladders board for four paper games: kinship riddles, error hunts, preposition blanks, anagrams. the corrigé moved to p. 147, and every corrected exercise now prints its own score.

## voyons voir !

`Évaluez vos compétences dans toutes les situations vues jusqu'ici.` four blocks, two of them pointing back into [[fr/episode-5]], each closed by an `Évaluez-vous (✓)` grid (☺ 😐 ☹, one row per can-do statement) and a `Corrigez vos réponses` pointer into the corrigé, p. 147; the production exercises are graded `avec l'aide de votre collègue ou du personnel enseignant` instead.
...
- `Je peux utiliser les déterminants mon, ma et mes.`

### présenter quelqu'un ou quelque chose et faire connaissance avec quelqu'un
...
- `Je peux utiliser les présentatifs c'est et ce sont.`
- `Je peux utiliser les verbes avoir et être.`
- `Je sais poser des questions pour faire connaissance avec quelqu'un.`
- `Je sais quand utiliser tu et vous.`
...
- `Je sais utiliser les déterminants ordinaux.`
- `Je comprends des indications pour me déplacer dans un édifice.`
- `Je peux donner des indications pour se déplacer dans un édifice.`
...
- `Je peux distinguer une permission, une interdiction et une obligation.`
- `Je peux m'informer sur le fonctionnement d'un établissement.`
- `Je comprends les messages d'un établissement.`

## à vous de jouer !

`Mettez en pratique ce que vous avez appris jusqu'à maintenant.` no dé, no pion, no board this season: four games on paper.[^reponses]
...
[^etage]: no `rez-de-chaussée` anywhere on the diagram: street level is already the `1er étage`, the `sous-sol` sits below it, and `5e étage (dernier étage)` caps a six-level building, basement included. that is the North-American count; the same tower in France would start at the rez-de-chaussée and put its `1er étage` one flight up, so every floor number here reads one higher than a hexagonal reader expects.

[^musique]: the plan prints one large room `Musique 105` with two small rooms `106` and `107` beside it (a guitar drawn in 106, a piano in 107), yet the consigne sends you to `la salle de musique 107`: the whole 105 à 107 corner is the music wing, and the numbered practice rooms inherit the name.
